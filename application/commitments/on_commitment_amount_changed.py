...
from model.commitments.events import CommitmentAmountChangedEvent


class OnCommitmentAmountChangedUpdateCommitmentReadModelHandler(EventHandler):
    ...
    def handle(self, event: CommitmentAmountChangedEvent):
        super().handle(event)
        if not isinstance(event, CommitmentAmountChangedEvent):
            return
        commitment = self.__commitment_read_model_repo.get_by_id(str(event.aggregate_id))
        if commitment:
            commitment.amount = event.amount
            commitment.unit = str(event.unit)
